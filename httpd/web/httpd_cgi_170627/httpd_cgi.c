...
BOOL GetHtmlWithSSI( char *file_path, char *dst_buf )
{
	...
	//Write_file("/tmp/httpdcgi_log", "GetHtmlWithSSI(): ", strlen("GetHtmlWithSSI(): "));
	//Write_file("/tmp/httpdcgi_log", file_path, strlen(file_path));
	...
	while( fgets(line_buf, sizeof(line_buf), fp) )
	{
		...
		//Write_file("/tmp/httpdcgi_log", line_buf, strlen(line_buf));
...
int uci_httpd_convert(char * uci_head, char *param)
{
	...
	for( i = 0; i < param_len; i ++ )
	{
		if( (pfind[i] == '&') ) 
		{
			...
			cgi_string_convert(pfind_hdr, uci_buf, sizeof(uci_buf));
...
//int cgi_string_convert(char *inputstring, char *rep, int rep_len)
...
int writefile_httpd_convert(char * writefile_head, char *param)
{
	...
	pfind = param;
	pfind_hdr = param;
	//printf("writefile_httpd: %s, %s \n",  param, writefile_head);
...
int DecodeAndProcessData(char *uri, char *input)
{
	...
	{ 
		...
		//Write_file("/tmp/httpdcgi_log", "DecodeAndProcessData", strlen("DecodeAndProcessData"));
	}
	// run shell cmd
	else if(uci_hdr[0] == '~')
	{
	...
	Write_file("/tmp/httpdcgi_log", "DecodeAndProcessData debug \n", strlen("DecodeAndProcessData debug \n"));
